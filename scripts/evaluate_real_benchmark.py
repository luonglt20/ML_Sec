#!/usr/bin/env python3
"""100% Real API Statistical Evaluation Benchmark for MedQA (1270 questions).

Executes 100% REAL API calls in parallel (30 workers) across V0, V1, V2, V3, V4.
Computes exact empirical statistics:
  - Table 1: Per-Variant Metrics (Correct/1270, Accuracy, Invalid Rate, Avg Tokens, Latency, Total Cost)
...
def process_case(args_tuple):
    case_idx, case, v, config, llm_client, retriever = args_tuple
...
def run_real_eval(sample_size: int, dataset_file: str, max_workers: int = 30):
    print("=" * 105)
    print(f"  100% REAL API STATISTICAL BENCHMARK — {sample_size} QUESTIONS FROM data/{dataset_file}")
    print("=" * 105)
    ...
    actual_n = len(cases)
    print(f"[Dataset] Loaded {actual_n} real MedQA test questions.")
    ...
    for v in variants:
        print("\n" + "-" * 85, flush=True)
        print(f"  EXECUTING REAL API CALLS FOR VARIANT: {v} ({max_workers} PARALLEL THREADS)", flush=True)
        print("-" * 85, flush=True)

        task_tuples = [(i, case, v, config, llm_client, retriever) for i, case in enumerate(cases)]
        results = [None] * actual_n
        completed = 0
        t_start_v = time.monotonic()

        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_map = {executor.submit(process_case, tup): tup[0] for tup in task_tuples}
            for future in as_completed(future_map):
                idx = future_map[future]
                res = future.result()
                results[idx] = res
                completed += 1
                if completed % 100 == 0 or completed == actual_n:
                    pct = (completed / actual_n) * 100.0
                    print(f"  • [{v}] Progress: {completed}/{actual_n} ({pct:.1f}%) processed...", flush=True)

        t_elapsed_v = time.monotonic() - t_start_v

        correct = sum(1 for r in results if r["is_corr"])
        invalid = sum(1 for r in results if not r["is_valid"])
        total_prompt_tok = sum(r["prompt_tokens"] for r in results)
        total_comp_tok = sum(r["completion_tokens"] for r in results)
        total_latency = sum(r["latency"] for r in results)
        scores = [1 if r["is_corr"] else 0 for r in results]

        acc = (correct / actual_n) * 100.0
        invalid_rate = (invalid / actual_n) * 100.0
        avg_tok = (total_prompt_tok + total_comp_tok) / actual_n
        avg_lat = total_latency / actual_n
        cost = compute_cost(total_prompt_tok, total_comp_tok)
        ...
        print(f"  >> [{v} Real Summary] Accuracy: {correct}/{actual_n} ({acc:.2f}%) | Time: {t_elapsed_v:.1f}s | Invalid: {invalid_rate:.2f}% | Avg Latency: {avg_lat:.2f}s", flush=True)

    print("\n" + "=" * 105)
    print(f"TABLE 1: OVERALL VARIANT METRICS (N = {actual_n} REAL API EVALUATIONS)")
    print("=" * 105)
...
if __name__ == "__main__":
    ...
    run_real_eval(args.n, args.file, args.workers)

#!/usr/bin/env python3
"""Statistical Evaluation Benchmark for MedQA (1273 test cases).

Computes all criteria specified in evaluation criteria:
  Table 1: Per-Variant Metrics
    - Variant name
    - Correct / N (e.g. 1273)
    - Accuracy (%)
    - Invalid Rate (%)
  ...
  Table 2: Pairwise Comparison Metrics with Statistical Tests
    - Comparison (A vs B)
    - Acc A (%)
    - Acc B (%)
    - Delta (%)
...
# Full index or fallback passages dict
PASSAGES_DICT = {
...
def calculate_mcnemar_p_value(b: int, c: int) -> float:
    ...
    p_val = erfc(sqrt(stat / 2.0))
    return p_val


def calculate_paired_ci(diffs: list[float]) -> tuple[float, float]:
    """Calculate 95% confidence interval for paired accuracy delta."""
    n = len(diffs)
    if n <= 1:
        return (0.0, 0.0)
    mean_d = sum(diffs) / n
    ...
    se_d = sqrt(var_d / n)
    ci_low = (mean_d - 1.96 * se_d) * 100.0
    ci_high = (mean_d + 1.96 * se_d) * 100.0
    return (ci_low, ci_high)


def compute_cost(prompt_tokens: int, completion_tokens: int) -> float:
    """Estimate total API cost in USD based on standard rates."""
    cost_prompt = (prompt_tokens / 1_000_000) * 0.59
    cost_completion = (completion_tokens / 1_000_000) * 0.79
    return cost_prompt + cost_completion


def run_eval(sample_size: int, dataset_file: str):
    print("=" * 100)
    print(f"  STATISTICAL EVALUATION BENCHMARK — {sample_size} QUESTIONS ({dataset_file})")
    print("=" * 100)
    ...
    actual_n = len(cases)
    print(f"[Dataset] Evaluated on N = {actual_n} cases.\n")
    ...
    for v in variants:
        print(f"Running evaluation for Variant {v}...")
        correct = 0
        invalid = 0
        total_prompt_tok = 0
        total_comp_tok = 0
        ...
        for idx, case in enumerate(cases):
            q_id = case.get("question_id", f"case-{idx:05d}")
            ...
            try:
                ...
                if is_corr:
                    correct += 1
                scores.append(1 if is_corr else 0)
        ...
        avg_tok = total_tok / actual_n
        avg_lat = total_latency / actual_n
        total_cost = compute_cost(total_prompt_tok, total_comp_tok)
    ...
    # -------------------------------------------------------------
    # TABLE 1: OVERALL VARIANT METRICS (Matches Image 1 Header)
    # -------------------------------------------------------------
    print("\n" + "=" * 100)
    print(f"TABLE 1: OVERALL VARIANT METRICS (N = {actual_n})")
    print("=" * 100)
    ...
    # -------------------------------------------------------------
    # TABLE 2: PAIRWISE COMPARISON METRICS WITH STATISTICAL TESTS (Matches Image 2 Header)
    # -------------------------------------------------------------
    print("\n" + "=" * 100)
    ...
    for vA, vB in pairs:
        ...
        p_val = calculate_mcnemar_p_value(b=loss, c=win)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n", type=int, default=1273, help="Sample size (default: 1273)")
    parser.add_argument("--file", type=str, default="test.jsonl", help="Dataset file (default: test.jsonl)")
    args = parser.parse_args()
    run_eval(args.n, args.file)
